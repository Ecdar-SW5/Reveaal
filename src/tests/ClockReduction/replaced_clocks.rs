...
pub mod test {
    ...
    pub fn test_test() {
        ...
        expected_locations.insert(redundant_clocks[0].clock.to_string(),HashSet::from([]));
        expected_locations.insert(redundant_clocks[1].clock.to_string(),HashSet::from([]));
        expected_locations.insert(global_clock[0].to_string(),HashSet::from([]));
